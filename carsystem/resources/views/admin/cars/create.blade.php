...
<div class="container mt-5">
    ...
    <form action="{{ route('admin.cars.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" class="form-control @error('CAR_NAME') is-invalid @enderror" id="CAR_NAME" name="CAR_NAME" value="{{ old('CAR_NAME') }}" required>
        @error('CAR_NAME')
        ...
        <div class="mb-3">
            ...
            <input type="file" class="form-control @error('CAR_IMG') is-invalid @enderror" 
                   id="CAR_IMG" name="CAR_IMG" accept="image/*" required>
            @error('CAR_IMG')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
